  ...
  if ('modelFetch' in field) {
    ...
    if (field.validateObjectArray === true) {
      async.map(liveValue, function(item, itemDone) {
        if (item === null) {
          return itemDone(new verror.VError({
            info:{
              badValue: liveValue
            }
          }, "'null' value encountered", liveValue), null);
        }
      ...
      exports.validateObject(liveValue, field.objectFields, function(err, result) {
...
exports.validateObject = function(obj, spec, callback) {
  ...
  if (validFieldNames.indexOf('ignoreFieldsField') !== -1) {
    validFieldNames.splice(validFieldNames.indexOf('ignoreFieldsField'), 1);
  }
  return async.each(validFieldNames, function(fieldName, eachCallback) {
    if (fieldName === 'ignoreFieldsField') {
      return eachCallback()
    }
    ...
    if ('values' in fieldSpec) {
      ...
      let liveValue = obj[fieldName];
      if (Array.isArray(liveValue)) {
        if (!fieldSpec.ensureArray) {
          let error = new verror.VError("The value must be an object");
          let fieldError = new FieldValidationError({
            cause: error,
            info: {
              fieldName: fieldName
        ...
        return async.map(liveValue, function(val, mapCallback) {
          ...
          if ((fieldSet.fields) && (fieldSet.fields.rawValue)) {
            fieldSet.fields.rawValue.required = true;
          }
          ...
          function(err, parsedValue) {
            ...
            let returnVal = null;
            if ((val !== undefined) && (val !== null) && (typeof val !== 'object')) {
              returnVal = parsedValue.rawValue;
            } else {
              returnVal = parsedValue;
            }
            return mapCallback(null, returnVal);
          })
        }, function(err, results) {
          if (err !== null) {
            errorList.push(err);
          ...
          let parsedArr = results;
          spec.fields[fieldName].parsedValue = parsedArr;
          resultObj[fieldName] = parsedArr;
          return eachCallback(null);
        })
      } else {
        if (fieldSpec.ensureArray) {
          let error = new verror.VError("The value must be an array");
          let fieldError = new FieldValidationError({
            cause: error,
            info: {
              fieldName: fieldName
    ...
    } else if (('fields' in fieldSpec) && (fieldSpec.ignoreFieldsField !== true)) {
      if ((obj[fieldName] === undefined) && (fieldSpec.required)) {
        let error = new verror.VError("A value is required");
        let fieldError = new FieldValidationError({
          cause: error,
